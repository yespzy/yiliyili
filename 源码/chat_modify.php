...
if(isset($_GET['id'])){
  $id = intval($_GET['id']);
  $query = mysqli_query($conn, "SELECT * FROM `reply` WHERE `id` = '$id'");
...
if(!empty($_POST['sub_chat'])){
	...
	$query = mysqli_query($conn, "UPDATE `reply` SET  `Keyword`= '$Keyword',`Reply`= '$Reply' WHERE `id`= '$id'");
						...
						<div class="col-md-12 box_content">
							<div class="box_content_1">
								<div class="content_title">
									<h2><img src="img/pen.png" />修改消息内容</h2>
								</div>
								<span></span>
								<div class="content_text">
									<div class="content" align="center">
										<!--表单开始-->
										<form class="form-horizontal" onsubmit="return checkForm();" method="post" enctype="multipart/form-data" action="chat_modify.php?id=<?php echo $id; ?>">
											<div class="form-group">
												<div class="col-sm-12 box_form">
													<input class="form-control" name="Keyword" type="text" id="news_title" placeholder="点击输入关键词" value="<?php echo ''.$row[1].''; ?>" onkeyup="checkTitle();">
													<span class="popover-show titleMsg" data-container="body" data-toggle="popover" data-placement="bottom"
													 data-content="关键词不能为空"></span>
												</div>
											</div>
											<div class="form-group">
												<div class="col-sm-12 box_form">
													<textarea class="form-control" name="Reply" rows="20" cols="80" type="Text" id="news_content"
													 placeholder="请输入内容" onkeyup="checkContent();"><?php echo ''.$row[2].''; ?></textarea>
											...
											<div class="form-group">
												...
												<input type="submit" name="sub_chat" class="btn btn-default" value="提交修改">
												</div>
											</div>
										</form>
										<!--表单结束-->
