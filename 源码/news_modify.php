...
if(!empty($_POST['sub_news'])){
	...
	//echo "<script>alert('编号： $id \\n标题：$title \\n时间：$time \\n作者：$author \\n图片路径：$img \\n类型：$type');</script>";
	$query = mysqli_query($conn, "UPDATE `news` SET `title`= '$title',`content`= '$content',`author`= '$author',`time`= '$time',`img`= '$img',`type`= '$type' WHERE `id`= '$id'");
	echo "<script>alert('修改成功'); location.href='backstage.php';</script>";
}
?>
							...
							<div class="box_content_1">
								...
								<div class="content_text">
									<div class="content" align="center">
										<!--表单开始-->
										<form class="form-horizontal" onsubmit="return checkForm();" method="post" enctype="multipart/form-data" action="news_modify.php?id=<?php echo $id; ?>">
											<div class="form-group">
												<div class="col-sm-12 box_form">
													<input class="form-control" name="title" type="text" id="news_title" placeholder="点击输入新闻标题" value="<?php echo ''.$row[1].''; ?>" onkeyup="checkTitle();">
											...
											<div class="form-group">
												...
												<div class="col-sm-12 btn-group" data-toggle="buttons">
												    ...
												    <label class="btn btn-default col-sm-3">
												        <input type="radio" name="news_type" id="option2" value="防控知识"> 防控知识
												    </label>
												    <label class="btn btn-default col-sm-3">
											...
											<div class="form-group">
												<div class="col-sm-12 box_form">
													<textarea class="form-control" name="content" rows="20" cols="80" type="Text" id="news_content"
													 placeholder="点击输入新闻内容" onkeyup="checkContent();"><?php echo ''.$row[2].''; ?></textarea>
											...
											<div class="form-group">
												...
												<input type="submit" name="sub_news" class="btn btn-default" value="提交修改">
												</div>
											</div>
										</form>
										<!--表单结束-->
